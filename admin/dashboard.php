...
// Total tests passés
$stmt = $pdo->query('SELECT COUNT(*) FROM resultats');
$total_tests = $stmt->fetchColumn();

// Score moyen global
$stmt = $pdo->query('SELECT ROUND(AVG(pourcentage), 1) FROM resultats');
$score_moyen = $stmt->fetchColumn() ?? 0;

// Nombre d'intervenantes actives
$stmt = $pdo->query("SELECT COUNT(*) FROM utilisateurs WHERE role = 'intervenante' AND actif = 1");
$total_intervenantes = $stmt->fetchColumn();

// Scores insuffisants (< 50%)
$stmt = $pdo->query("SELECT COUNT(*) FROM resultats WHERE mention = 'insuffisant'");
$scores_faibles = $stmt->fetchColumn();

// Derniers résultats (5 plus récents)
$stmt = $pdo->query(
    'SELECT r.id, u.prenom, u.nom, t.titre, r.pourcentage, r.mention, r.passe_le
     FROM resultats r
...
<div class="fc-container">
    ...
    <div style="display:grid; grid-template-columns:repeat(4,1fr); gap:16px; margin-bottom:36px;">
        ...
        <!-- Total tests passés -->
        <div style="background:#fff; border-radius:16px; padding:22px; border:1px solid #E5DDD0; transition:all .2s;"
             onmouseover="this.style.boxShadow='0 6px 24px rgba(27,43,58,.1)'; this.style.transform='translateY(-2px)'"
             onmouseout="this.style.boxShadow='none'; this.style.transform='none'">
            <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:14px;">
                <span style="font-size:11px; font-weight:700; letter-spacing:1.5px; text-transform:uppercase; color:#8A9BAD;">Tests passés</span>
            ...
            <div style="font-size:12px; color:#8A9BAD; font-weight:300; margin-top:4px;">évaluations réalisées</div>
        </div>
        ...
        <div style="background:#fff; border-radius:16px; padding:22px; border:1px solid #E5DDD0; transition:all .2s;"
             ...
             onmouseout="this.style.boxShadow='none'; this.style.transform='none'">
            <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:14px;">
                <span style="font-size:11px; font-weight:700; letter-spacing:1.5px; text-transform:uppercase; color:#8A9BAD;">Score moyen</span>
            ...
            <div style="font-size:12px; color:#8A9BAD; font-weight:300; margin-top:4px;">sur l'ensemble des tests</div>
        </div>
        ...
        <div style="background:#fff; border-radius:16px; padding:22px; border:1px solid #E5DDD0; transition:all .2s;"
             ...
             onmouseout="this.style.boxShadow='none'; this.style.transform='none'">
            <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:14px;">
                <span style="font-size:11px; font-weight:700; letter-spacing:1.5px; text-transform:uppercase; color:#8A9BAD;">Intervenantes</span>
            ...
            <div style="font-size:12px; color:#8A9BAD; font-weight:300; margin-top:4px;">comptes actifs</div>
        </div>
        ...
        <div style="background:#fff; border-radius:16px; padding:22px; border:1px solid #E5DDD0; transition:all .2s;"
             ...
             onmouseout="this.style.boxShadow='none'; this.style.transform='none'">
            <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:14px;">
                <span style="font-size:11px; font-weight:700; letter-spacing:1.5px; text-transform:uppercase; color:#8A9BAD;">Alertes</span>
            ...
            <div style="font-size:12px; color:#8A9BAD; font-weight:300; margin-top:4px;">scores insuffisants</div>
        </div>

    </div>

    <!-- ===== DERNIERS RÉSULTATS ===== -->
    <div style="background:#fff; border-radius:18px; border:1px solid #E5DDD0; overflow:hidden; margin-bottom:32px;">

        <!-- Header du tableau -->
        <div style="padding:20px 24px; border-bottom:1px solid #E5DDD0; display:flex; align-items:center; justify-content:space-between;">
            <div>
                <h2 style="font-family:'Cormorant Garamond',serif; font-size:20px; font-weight:700; color:#1B2B3A; margin-bottom:2px;">Derniers résultats</h2>
                <p style="font-size:12px; color:#8A9BAD; font-weight:300;">5 évaluations les plus récentes</p>
            </div>
            <a href="<?= BASE_URL ?>admin/resultats.php" style="font-size:13px; color:#E8846A; text-decoration:none; font-weight:500;">
                Voir tout →
            </a>
        </div>
        ...
        <div style="padding:48px; text-align:center; color:#8A9BAD;">
            <div style="font-size:40px; margin-bottom:12px;">📋</div>
            <p style="font-weight:300;">Aucun résultat pour l'instant.</p>
            <a href="<?= BASE_URL ?>admin/tests.php" style="display:inline-block; margin-top:16px; color:#E8846A; font-size:13px; font-weight:500;">
        ...
        <table style="width:100%; border-collapse:collapse;">
            <thead>
                <tr style="background:#F5F0E8;">
                    <th style="padding:12px 24px; font-size:11px; font-weight:700; letter-spacing:1px; text-transform:uppercase; color:#8A9BAD; text-align:left;">Intervenante</th>
                    <th style="padding:12px 24px; font-size:11px; font-weight:700; letter-spacing:1px; text-transform:uppercase; color:#8A9BAD; text-align:left;">Test</th>
                    <th style="padding:12px 24px; font-size:11px; font-weight:700; letter-spacing:1px; text-transform:uppercase; color:#8A9BAD; text-align:center;">Score</th>
                    <th style="padding:12px 24px; font-size:11px; font-weight:700; letter-spacing:1px; text-transform:uppercase; color:#8A9BAD; text-align:center;">Mention</th>
                    <th style="padding:12px 24px; font-size:11px; font-weight:700; letter-spacing:1px; text-transform:uppercase; color:#8A9BAD; text-align:left;">Date</th>
                </tr>
            </thead>
            <tbody>
                ...
                ?>
                <tr style="border-bottom:1px solid #E5DDD0; transition:background .15s;"
                    onmouseover="this.style.background='#F5F0E8'"
                    onmouseout="this.style.background='transparent'">
                    <td style="padding:14px 24px; font-size:14px; font-weight:500; color:#1B2B3A;">
                        <?= htmlspecialchars($r['prenom'] . ' ' . $r['nom']) ?>
                    </td>
                    <td style="padding:14px 24px; font-size:13px; font-weight:300; color:#3D5166;">
                    ...
                    <td style="padding:14px 24px; font-size:12px; color:#8A9BAD; font-weight:300;">
    ...
    <div style="display:grid; grid-template-columns:repeat(3,1fr); gap:16px;">
        ...
        <a href="<?= BASE_URL ?>admin/tests.php"
           style="background:#fff; border-radius:16px; padding:22px; border:1px solid #E5DDD0; text-decoration:none; display:flex; align-items:center; gap:14px; transition:all .2s;"
           onmouseover="this.style.boxShadow='0 6px 24px rgba(27,43,58,.1)'; this.style.transform='translateY(-2px)'"
           onmouseout="this.style.boxShadow='none'; this.style.transform='none'">
            <div style="width:44px; height:44px; background:#FBF0EC; border-radius:12px; display:flex; align-items:center; justify-content:center; flex-shrink:0;">
                <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#E8846A" stroke-width="2" stroke-linecap="round"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
            </div>
            <div>
                <div style="font-size:14px; font-weight:600; color:#1B2B3A; margin-bottom:3px;">Gérer les tests</div>
                <div style="font-size:12px; color:#8A9BAD; font-weight:300;">Créer, modifier, supprimer</div>
            </div>
        </a>
        ...
        <a href="<?= BASE_URL ?>admin/utilisateurs.php"
           ...
           onmouseout="this.style.boxShadow='none'; this.style.transform='none'">
            ...
            <div>
                <div style="font-size:14px; font-weight:600; color:#1B2B3A; margin-bottom:3px;">Intervenantes</div>
                <div style="font-size:12px; color:#8A9BAD; font-weight:300;">Gérer les comptes</div>
            </div>
        </a>
        ...
        <a href="<?= BASE_URL ?>admin/resultats.php"
           ...
           onmouseout="this.style.boxShadow='none'; this.style.transform='none'">
            ...
            <div>
                <div style="font-size:14px; font-weight:600; color:#1B2B3A; margin-bottom:3px;">Tous les résultats</div>
                <div style="font-size:12px; color:#8A9BAD; font-weight:300;">Statistiques complètes</div>
            </div>
        </a>
